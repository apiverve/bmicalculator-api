...
class BmicalculatorValidationException implements Exception {
  final List<String> errors;

  BmicalculatorValidationException(this.errors);

  @override
  String toString() => 'BmicalculatorValidationException: ${errors.join("; ")}';
}
...
/// BMI Calculator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/bmicalculator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [weight] (required) - The weight of the person in either kg or lb [min: 1]
/// * [height] (required) - The height of the person in cm or ft [min: 1]
/// * [unit] (required) - The unit of measurement for weight and height
class BmicalculatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'weight': ValidationRule(type: 'number', required: true, min: 1),
    'height': ValidationRule(type: 'number', required: true, min: 1),
    'unit': ValidationRule(type: 'string', required: true),
  };

  BmicalculatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bmicalculator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BmicalculatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BmicalculatorValidationException(errors);
    }
  }

  /// Execute a request to the BMI Calculator API.
  ///
  /// Parameters are validated before sending the request.
  Future<BmicalculatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BmicalculatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BmicalculatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BmicalculatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BmicalculatorException('Resource not found');
      } else {
        throw BmicalculatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BmicalculatorException) rethrow;
      throw BmicalculatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the BMI Calculator API client.
class BmicalculatorException implements Exception {
  final String message;

  BmicalculatorException(this.message);

  @override
  String toString() => 'BmicalculatorException: $message';
}
