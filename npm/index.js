...
class bmicalculatorWrapper {

    constructor(options) {
        if (!options || typeof options !== 'object') {
            throw new Error('Options object must be provided. See documentation: https://docs.apiverve.com/ref/bmicalculator');
        }
    ...
    async execute(query, callback) {
        ...
        var requiredParams = ["weight","height","unit"];
        if (requiredParams.length > 0) {
            for (var i = 0; i < requiredParams.length; i++) {
                if (!query[requiredParams[i]]) {
                    throw new Error(`Required parameter [${requiredParams[i]}] is missing. See documentation: https://docs.apiverve.com/ref/bmicalculator`);
                }
            }
        }
...
module.exports = bmicalculatorWrapper;
